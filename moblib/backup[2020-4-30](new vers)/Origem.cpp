...
int main(int argc, char **argv) {
	...
	mlMovimentInstance targetm;
	targetm.x = 500;
	targetm.y = 500;
	mlFormat target(&targetm, 40, 40);
	
	mlMovimentAuto* arrowm =(mlMovimentAuto*) calloc(sizeof(mlMovimentAuto),5);
	
	arrowm[0] = mlMovimentAuto(0, 0);
	arrowm[0] = mlMovimentAuto(0, 0);
	arrowm[0] = mlMovimentAuto(0, 0);
	arrowm[0] = mlMovimentAuto(0, 0);
	arrowm[0] = mlMovimentAuto(0, 0);

	mlFormat arrow(pers.moviment, 5, 5);
	...
	while (!parar) {
		...
		if (ev.type == ALLEGRO_EVENT_TIMER && al_is_event_queue_empty(fila)) {
			...
			if (mlIsolatedCollision(&target, &objmouse) && mouse_is_pressed && arrowm[0].stop) {
				 mlDirection(&arrowm[0], targetm.x, targetm.y, 12);
				
			}
			else if(arrowm[0].stop){
				arrowm[0].x = pers.moviment->x - pers.sheet->handleX[4];
				arrowm[0].y = pers.moviment->y - pers.sheet->handleY[4];
			}
			else {
				mlRun(&arrowm[0]);
			}
		...
		al_draw_filled_rectangle(targetm.x,targetm.y, targetm.x + target.width , targetm.y + target.height,purple);

		if(!arrowm[0].stop)al_draw_filled_rectangle(arrowm[0].x, arrowm[0].y, arrowm[0].x + arrow.width, arrowm[0].y + arrow.height,gold);
		// atualiza a tela
		al_flip_display();
