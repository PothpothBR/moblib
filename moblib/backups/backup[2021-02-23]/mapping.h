...
inline int overdraw(MAPPING* map, MOVIMENT::INSTANCE* self, SHEET *sheet, int x, int y) {
	return (self->x + x * sheet->cellWidth + sheet->cellWidth  < map->lx ||
		...
		self->x + x * sheet->cellWidth  > map->lw ||
		self->y + y * sheet->cellHeight > map->lh);
}
...
// nao testado
void loadmapanim(MAPPING* self, ANIM::ANIMATE* anim, char key,  std::string name) {
