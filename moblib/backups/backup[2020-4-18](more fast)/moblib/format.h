...
#define mlCOLIDON true
#define mlCOLIDOFF false
...
public:
	...
	float *x, *y,
		  width, height;
	bool *colid = nullptr;
	bool colidState = mlCOLIDON;

	void  setColidState(bool verify) { colidState = verify; }
	...
	mlFormat(mlMovimentInstance *selfMoviment, mlSheet *selfSheet) {
		...
		this->width = selfSheet->width();
		this->height = selfSheet->height();
...
void mlIsolatedCollision(mlFormat *self) {
	...
	if (self->getColidState() == mlCOLIDOFF) return;

	float
		x1 = *self->x, y1 = *self->y,
		w1 = self->width, h1 = self->height;
	...
	for (mlFormat *selfver = self->head;selfver;selfver = selfver->next) {
		...
		float
			x2 = *selfver->x, y2 = *selfver->y,
			w2 = selfver->width, h2 = selfver->height;
...
bool mlIsolatedCollision(mlFormat *self, float x, float y, float w, float h) {
	...
	if (self->getColidState() == mlCOLIDOFF)
		return false;
...
void mlCollision(mlFormat *self) {
	...
	for (self = head;self;self = self->next) {
		...
		if (self->getColidState() == mlCOLIDOFF) continue;
