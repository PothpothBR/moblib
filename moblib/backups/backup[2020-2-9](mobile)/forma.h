...
struct SpriteFormat
{
  ...
  public:
	...
	SpriteFormat(SpriteMovimentStance * selfMoviment, SpriteSheet * selfSheet)
	{
...
// testa a colisao de quadrados entre um objeto <form> e os outros objetos
// <form>
void isolatedCollision(SpriteFormat * self)
{
	...
	for (SpriteFormat * selfver = self->getHead(); selfver; selfver = selfver->getNext())
	{
		...
		float
			x2 = selfver->getX(),
			y2 = selfver->getY(), w2 = selfver->getWidth(), h2 = selfver->getHeight();
...
void testcriclcold(float x1, float y1, float r1, float x2, float y2, float r2)
{
	...
	if (d < r1 + r2)
		std::cout << "!!!!!!!!colidiu pouuurrrrrra!!!!!!!!" << std::endl;

}

/* void testcriclcoldcquad(float x1, float y1, float r1, float x2, float y2,
   float w2, float h2) {
   ...
   if (d < r1 + r2) std::cout << "!!!!!!!!colidiu pouuurrrrrra!!!!!!!!" <<
   std::endl;
