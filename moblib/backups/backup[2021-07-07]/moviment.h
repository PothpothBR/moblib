...
namespace MOB_MOVIMENT {
	...
	public:
		...
		INSTANCE_GHOST(float x, float y, sint trace_size = 0, sint trace_delay = 0) : INSTANCE(x, y) {
			if (trace_size) {
	...
	public:
		...
		~KEYBOARD() {if(position) delete position; }
	};
	...
	public:
		...
		~AUTO() { if (position) delete position; }
	};
	...
	public:
		...
		~MOUSE() { if (position) delete position; }
	};
	...
	public:
		...
		~TOUCH() { if (position) delete position; }
	};
}

using namespace MOB_MOVIMENT;

float mob_get_moviment_trace_X(INSTANCE_GHOST* self, sint index) { if (index < self->trace_size) return self->trace_X[index];  return -100; }
float mob_get_moviment_trace_Y(INSTANCE_GHOST* self, sint index) { if (index < self->trace_size) return self->trace_Y[index];  return -100; }

void mob_set_moviment_trace(INSTANCE_GHOST* self, float x, float y) {
...
void mob_set_auto_trace_xy(INSTANCE_GHOST* self, float x, float y) {
	self->x = x, self->y = y;
...
void mob_set_auto_trace(INSTANCE_GHOST* self) {
	if (self->trace_delay_count > 1) { self->trace_delay_count--; return; }
	self->trace_delay_count = self->trace_delay;
...
// metodo para movimentar
template <class T>
void mob_run(KEYBOARD<T>* self, MOB_COLLISION::COLID* self_box, MOB_COLLISION::COLID* another_box, int obj_in_out, bool all = true) {
	self_box->reload();
	mob_run(self, all);
