...
// gera um cópia identica do mapa
...
MOB_FORM_CHAIN* mob_map_fit_rectangles(MOB_MAP* self)  {
	...
	// cria valores para enchugar a iteraçao
	...
	while (true) {
		...
		for (register int y = start_y; y < start_height; y++) {
			for (register int x = start_x; x < start_width; x++) {
				...
				// verifica se é o inicio do bloco de colizoes, primeiro em cima a esquerda
...
// os retangulos tem suas posicoes fixas em relaçao ao mapa e nao a tela, mas as mesmas nao atualizam seguindo a posicao do mapa
