...
/*
		//desenha o mapa de acordo com os tiles
		...
		THREAD *get = channel->head[channel->getChanelById(22)];// pega o canal de threads especifico
		for (int i = 0;i < channel->size[channel->getChanelById(22)];i++) {// desenha todo o canal
			ranged_atack *tmp = (ranged_atack*)get->obj;
		...
		al_draw_filled_rectangle(*test->x,*test->y,*test->x + test->width, *test->y + test->height, red);
		al_draw_filled_rectangle(player1->moviment->pos->x, player1->moviment->pos->y, player1->moviment->pos->x + player1->format->width, player1->moviment->pos->y + player1->format->height, blue);
