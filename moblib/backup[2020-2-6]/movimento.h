...
public:
	...
	SpriteMovimentStance(float x, float y, usint traceSize=0) {
		this->x = x;
		this->y = y;
	...
	void setTrace(float x,float y){
		if (!traceSize) return;
		for (usint i = traceSize-2,ir = traceSize-1;i >= 0;i--) {
			traceX[ir] = traceX[i];
			traceY[ir] = traceY[i];
		}
		traceX[0] = x;
	...
	usint getTraceX(usint index) { if (index < traceSize) return traceX[index]; }
	usint getTraceY(usint index) { if (index < traceSize) return traceY[index]; }
...
public:
	...
	SpriteMovimentKeys(float x, float y, usint vel = 0,usint traceSize = 0): SpriteMovimentStance(x, y, traceSize) {
...
struct SpriteMovimentClick : public SpriteMovimentStance {
	...
	SpriteMovimentClick(float x, float y,usint traceSize = 0) : SpriteMovimentStance(x, y, traceSize) {
...
struct SpriteMoviment : public SpriteMovimentStance {
	...
	SpriteMoviment(float x, float y, usint traceSize = 0) : SpriteMovimentStance(x, y, traceSize) {
