...
int& mod(MAP *self, int x, int y, int l) {
	if (x >= self->width || y >= self->height || l >= self->cellSize[index(x, y, self->width)]) { debugLog("error at get get - x:", x, "y:", y); }
	return *self->buffer[index(x, y, self->width)][l];
}

int& mod(MAP *self, int i, int l) {
	if (i >= self->width * self->height && l >= self->cellSize[i]) { debugLog("error at get get - index:", i); }
	return *self->buffer[i][l];
}
